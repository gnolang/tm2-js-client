...
const adaptResponseBase = (rb: ResponseBase): ABCIResponseBase => {
  return {
    Error: rb.error?.['@type']
      ? { '@type': rb.error['@type'], value: rb.error.value }
      : null,
...
const adaptBlockId = (
  bid: { hash: Uint8Array; parts: { total: bigint; hash: Uint8Array } } | null
) => {
  if (!bid) {
    return { hash: null, parts: { total: '0', hash: null } };
  }
  return {
  ...
  return {
    ...
    block: {
      ...
      last_commit: r.block.lastCommit
        ...
        : {
            block_id: { hash: null, parts: { total: '0', hash: null } },
            precommits: null,
          },
    },
  };
...
export const adaptBroadcastTxSyncResponse = (
  ...
  return {
    error: r.responseBase.error?.['@type']
      ? { '@type': r.responseBase.error['@type'], value: r.responseBase.error.value }
      : null,
